# Aptitude Mastery Plan

## Overview

This study plan is designed to build a strong foundation in **Quantitative Aptitude**, **Logical Reasoning**, and **Verbal Ability** over a period of **12 weeks**. It is suitable for campus placements, competitive examinations, and aptitude-based interviews.

---

# Daily Study Schedule

| Activity              | Recommended Duration |
| --------------------- | -------------------: |
| Learn Concepts        |        45–60 minutes |
| Solve Guided Examples |           20 minutes |
| Practice Questions    |           60 minutes |
| Review Errors         |           20 minutes |
| Timed Practice        |           20 minutes |

> **Recommended Study Time:** Approximately 2.5–3 hours per day. If time is limited, aim for at least 90 minutes while maintaining the same sequence.

---

# 12-Week Study Plan

| Week | Focus Area                               | Weekly Target                 |
| ---- | ---------------------------------------- | ----------------------------- |
| 1    | Number Systems & Divisibility            | 150–200 Questions             |
| 2    | Percentages, Profit & Loss, Discounts    | 150–200 Questions             |
| 3    | Ratios, Proportion, Averages, Mixtures   | 150–200 Questions             |
| 4    | Time, Speed & Distance                   | 150–200 Questions + Mock Test |
| 5    | Time & Work, Pipes & Cisterns            | 150–200 Questions             |
| 6    | Simple & Compound Interest               | 150–200 Questions             |
| 7    | Permutations, Combinations & Probability | 180–220 Questions             |
| 8    | Algebra & Equations                      | 180–220 Questions + Mock Test |
| 9    | Geometry & Mensuration                   | 180–220 Questions             |
| 10   | Data Interpretation                      | 15–20 Sets + Mock Test        |
| 11   | Logical Reasoning                        | 200–250 Questions             |
| 12   | Verbal Ability & Final Revision          | 3–5 Full-Length Mock Tests    |

---

# Recommended Learning Process

For every topic, follow the same structured approach:

1. Learn the underlying concepts.
2. Solve worked examples.
3. Practice basic problems.
4. Progress to intermediate questions.
5. Attempt advanced problems.
6. Solve timed practice sets.
7. Review mistakes and record them in an error log.

---

# Weekly Objectives

| Objective           | Target       |
| ------------------- | ------------ |
| Concepts Completed  | 100%         |
| Practice Questions  | 150–250      |
| Timed Practice Sets | 5–10         |
| Mock Tests          | As Scheduled |
| Accuracy            | Above 85%    |
| Revision            | Every Sunday |

---

# Logical Reasoning Topics

* Coding-Decoding
* Blood Relations
* Direction Sense
* Seating Arrangement
* Linear Arrangement
* Circular Arrangement
* Puzzle Solving
* Syllogisms
* Statement and Conclusion
* Statement and Assumption
* Cause and Effect
* Input-Output
* Ranking
* Calendar
* Clock Problems

---

# Verbal Ability Topics

* Reading Comprehension
* Para Jumbles
* Sentence Correction
* Fill in the Blanks
* Error Spotting
* Synonyms
* Antonyms
* Vocabulary
* Idioms and Phrases
* One-Word Substitution

---

# Mock Test Schedule

| Week | Activity                               |
| ---- | -------------------------------------- |
| 4    | Mock Test 1                            |
| 8    | Mock Test 2                            |
| 10   | Mock Test 3                            |
| 11   | Full-Length Mock Test                  |
| 12   | Three to Five Company-Level Mock Tests |

After every mock test:

1. Calculate overall accuracy.
2. Identify incorrect and skipped questions.
3. Categorize mistakes by topic.
4. Revise weak areas within 24 hours.
5. Reattempt incorrect questions after several days.

---

# Error Log Template

| Date | Topic | Question | Mistake | Correct Approach | Revised |
| ---- | ----- | -------- | ------- | ---------------- | ------- |
|      |       |          |         |                  |         |

Review the error log every Sunday to reinforce learning and avoid repeating mistakes.

---

# Recommended Resources

## Books
  - RS Aggarwal – Quantitative Aptitude
  - Arun Sharma – QA for CAT (Only if you are seriously preparing for CAT, this would be an additional benefit).
  - RS Aggarwal – Logical Reasoning
  - I've also uploaded another Aptitude Workbook (Purely Reasoning Ability) in the repo -> [PW Aptitude Workbook](https://github.com/sathvik-spartan/Aptitude_A-Z/blob/main/PW%20Aptitude%20Workbook.pdf)
> Note : Downloadable RS Aggarwal Books are in the "resources" folder

## Practice Platforms

  - [IndiaBix](https://www.indiabix.com)
  - [PrepInsta](https://prepinsta.com)
  - [Unstop](https://unstop.com/employers/online-assessment-platform/aptitude-test)
  - [Testbook](https://testbook.com)

---

# Best Practices

* Study consistently rather than relying on intensive short-term preparation.
* Prioritize conceptual understanding before learning shortcuts.
* Maintain an error log throughout the preparation period.
* Practice under timed conditions to improve speed.
* Analyze every mock test thoroughly.
* Focus on improving accuracy before attempting to increase speed.
* Revise previously studied topics every week.

---

# Expected Outcomes

By the end of this study plan, you should be able to:

* Solve aptitude problems efficiently and accurately.
* Perform well in campus placement assessments.
* Approach competitive examination aptitude sections with confidence.
* Identify recurring mistakes and systematically eliminate them through revision.

Consistent effort over the full 12-week period will produce significantly better results than irregular, high-intensity study sessions.

---

## Tips !   
- Practice regularly and avoid burnout.
- Maintain an **error log** for tricky questions. (Very Important)
- Time yourself to build speed and accuracy.
- Revise every Sunday for long-term retention.
- Be disciplined in your learning journey.

---

### Make sure to Star this repo if you found it helpful!  
Contributions, doubts (Gmail : sathvikspartan@gmail.com) , and improvements are always welcome!

---
